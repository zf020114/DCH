...
class PartialFC_V2(torch.nn.Module):
    ...
    def sample(self, labels, index_positive):
        ...
        with torch.no_grad():
            ...
            #就是返回一个和values一样大小的tensor,其中的元素是在sorted_sequence中大于等于values中值的索引，

        return self.weight[self.weight_index], labels
    ...
    def forward(
        ...
        if self.sample_rate < 1 and training:
        ...
        # logits = self.margin_softmax(logits, labels,training)
        # loss = self.dist_cross_entropy(logits, labels)
        return logits,labels

class PartialFC_V3(torch.nn.Module):
    """
    https://arxiv.org/abs/2203.15565
    A distributed sparsely updating variant of the FC layer, named Partial FC (PFC).
    When sample rate less than 1, in each iteration, positive class centers and a random subset of
    ...
    def __init__(
        ...
        sample_rate: float
            The rate of negative centers participating in the calculation, default is 1.0.
        """
        super(PartialFC_V3, self).__init__()
    ...
    def sample(self, labels, index_positive):
        ...
        return self.weight[self.weight_index], labels_pfc
    ...
    def forward(
        ...
        index_positive = labels < self.num_classes+1
        if self.sample_rate < 1 & is_train:
            weight, labels_resort,= self.sample(labels, index_positive)
        else:
            weight = self.weight 
            labels_resort = None
